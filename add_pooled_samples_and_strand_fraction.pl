...
### READ FIELDS
    ...
    my @iCol = (7,10,13,16,19);
    foreach my $i (0 .. 4) {
	next if ($hash{'GEN['.$i.'].FREQ'} == 0);
	$input_fields[$iCol[$i]] /= $hash{'GEN['.$i.'].DP'};
	$input_fields[$iCol[$i]] /= $hash{'GEN['.$i.'].FREQ'};
	$input_fields[$iCol[$i]] *= 10000;
	### +1 is a fudge!
	if ($input_fields[$iCol[$i]+1] > 100) {$input_fields[$iCol[$i]+1] = 100};
	$input_fields[$iCol[$i]] = sprintf("%.1f",$input_fields[$iCol[$i]]);
    }
...
sub insert_normal_tumor_columns {
    ...
    my $normal_column_name = 'GEN[2].ADF';
    my $tumor_column_name = 'GEN[4].ADF';
